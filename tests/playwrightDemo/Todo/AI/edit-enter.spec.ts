...
test.describe('Edit Todo Items', { tag: ['@Todo', '@Edit'] }, () => {
  ...
  test('Edit todo item and save via Enter',
    { tag: ['@smoke'] },
    async ({ page }) => {
    await allure.epic('Epic: Playwright Demo');
    await allure.feature('Feature: Todo MVC');
    await allure.story('Story: Edit and save via Enter');
    ...
    await allure.step('AND I clear the text and type "Buy cheese"', async (step) => {
    ...
    await allure.step('THEN the item is updated to "Buy cheese"', async (step) => {
      const buyCheese = page.locator('li').filter({ hasText: 'Buy cheese' });
      await expect(buyCheese, 'Item should be updated to "Buy cheese"').toBeVisible();
    });
    ...
    await allure.step('AND edit mode is exited', async (step) => {
      ...
      expect(isVisible, 'Edit input should not be visible').toBe(false);
    });

    // 7. And the updated item is visible in the list
    await allure.step('AND the updated item is visible in the list', async (step) => {
      const buyCheese = page.getByText('Buy cheese');
      await expect(buyCheese, 'Updated item should be visible').toBeVisible();
    });
  });
});
