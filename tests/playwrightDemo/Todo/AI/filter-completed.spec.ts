...
test.describe('Filter Todo Items', { tag: ['@Todo', '@Filter'] }, () => {
  ...
  test('Filter to show only completed items',
    { tag: ['@smoke'] },
    async ({ page }) => {
    await allure.epic('Epic: Playwright Demo');
    await allure.feature('Feature: Todo MVC');
    await allure.story('Story: Filter to show only completed items');
    ...
    // 2. When I click the 'Completed' filter
    await allure.step('WHEN I click the "Completed" filter', async (step) => {
    ...
    // 5. And the 'Completed' filter link is highlighted
    await allure.step('AND the "Completed" filter link is highlighted', async (step) => {
      const completedLink = page.getByRole('link', { name: 'Completed' });
      const className = await completedLink.evaluate(el => el.className);
      expect(className, 'Completed link should have selected class').toContain('selected');
    });
  });
});
